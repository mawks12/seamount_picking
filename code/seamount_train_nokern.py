...
points = SeamountHelp.readKMLbounds(Path('code/data/seamount_training_zone.kml').resolve())
data = SeamountHelp.readAndFilterGRD(Path('code/data') / 'vgg_swot.grd', points[:2], points[2:])

y_cents = pd.read_csv(Path('code/data') / 'all.xyhrdnc', sep=' ', header=None, names=['lat', 'lon', 'height', 'radius', 'depth', 'name', 'catalouge'])

X = SeamountHelp.seamount_radial_match(data.to_dataframe().reset_index(), y_cents)
split = SeamountCVSplitter(10)
X_traini, X_testi = next(split.split(X.drop(columns='Labels'), X['Labels']))
X_train = X.iloc[X_traini]
X_test = X.iloc[X_testi]
y_train = X_train['Labels'].to_numpy()
y_test = X_test['Labels'].to_numpy()
X_train = X_train.drop(columns=['Labels'])[['lat', 'lon', 'z']].to_numpy()
X_test = X_test.drop(columns=['Labels'])[['lat', 'lon', 'z']].to_numpy()
...
with open(Path('code/out') / 'remote_testing.pkl', 'wb') as fout:
    pickle.dump(grid, fout)

with open(Path('code/out') / 'remote_testing.txt', 'w') as f2out:
    f2out.write(
